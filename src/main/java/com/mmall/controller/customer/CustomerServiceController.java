...
public class CustomerServiceController {
    ...
    @ApiOperation(value = "获取商户名",  notes="需要Authorization")
    @GetMapping(value = "/userKeys",produces = {"application/json;charest=Utf-8"})
    public Result getUserKeys(){
        SysUserInfo userInfo = UserInfoConfig.getUserInfo();
        return customerServiceService.getUserKeys(userInfo);
    }

    @ApiOperation(value = "获取工单（0=全部，1=未处理，2=处理中，3=处理完毕）",  notes="需要Authorization")
    @GetMapping(value = "/list/{status}/{errorId}",produces = {"application/json;charest=Utf-8"})
    public Result<Page<ReplynumServiceDto>> getAllCustomerService(@PathVariable("status") Integer status,
                                                                  @PathVariable("errorId") Integer errorId,
        ...
        return customerServiceService.getAllCustomerServiceByUser( errorId,status,userInfo.getId(),ipage,waybillNumber);
    }
